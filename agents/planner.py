...
3. **Sharpness (清晰度)**: 
   - `laplacian_variance`: 拉普拉斯方差。值越低，图像越模糊、失焦；值过高可能是因为存在严重的噪点或过度锐化。
...
6. **Frequency & Noise (频率与噪声)**: 
   - `estimated_snr_db`: 盲信估计信噪比(dB)。值越低，图像噪点越严重；值越高，画面越纯净。注意！对于复杂图像，SNR并不能作为唯一判定依据，必须结合其他指标共同判定。
   - `high_frequency_ratio` / `total_variation`: 总变差与高频比例。结合 SNR 分析，SNR低且TV高通常意味着严重的噪点；SNR高且高频比例高则意味着细节丰富。

# Workflow
1. **多模态对齐（条件执行）**：如果用户输入了图像本身，请将视觉直观感受与量化指标交叉验证（例如：指标显示低 SNR，视觉上确认是否存在明显噪点）。如果未输入图像，请完全信任并基于量化指标进行逻辑推理。
2. **缺陷诊断**：找出严重偏离“高质量图像”常规阈值的指标，确定核心问题（如：严重欠曝、高噪点、对比度低、轻微模糊等）。
3. **输出描述**：以自然语言准确描述图像的基本情况，详细指出哪些指标和图像本身哪些地方出现了问题。
...
```
    ...
    def generate_code_stream(self, 
        ...
        llm_response = ''
        for t, chunk in self._call_llm_stream(user_intent, imgs=[user_img_rgb] if user_img_rgb is not None else None):
    ...
    def execute(self, user_intent: str, user_img_rgb: np.ndarray) -> dict:
        ...
        执行规划任务
        """
        raw_response = self._call_llm(user_intent, imgs=[user_img_rgb] if user_img_rgb else None)
